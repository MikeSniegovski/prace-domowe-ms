...
function previousAndNext(n) {
  let numParse = Number.parseInt(n);
  let numPrev = numParse - 1;
  let numNext = numParse + 1;
  return {
    previous: numPrev,
    next: numNext,
  }
...
function hasOneByOneDigit(n) {
  let numTest = n.toString();

  let num1 = Number.parseInt(numTest[0]);
  let num2 = Number.parseInt(numTest[1]);
  let num3 = Number.parseInt(numTest[2]);
